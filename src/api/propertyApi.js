...
export const getMailMerge = params => {
   return window.open(getUrl(`https://www.crm2print.com/api/merge/256813/token/wWJHcT/docx`, params));
};
export const findComparables = ({ propertyId, ...params }) => {
   return http({
      ...
      return data
         ...
         .map(item => {
            item.lastSaleDetail.timestamp = new Date(item.lastSaleDetail.contractDate).getTime();
            item.attributes.cars = Number(item.attributes.lockUpGarages || 0) + Number(item.attributes.carSpaces || 0);
            return item;
         });
   });
};
